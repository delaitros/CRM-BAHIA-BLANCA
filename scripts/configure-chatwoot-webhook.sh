...
# Requiere que ya tengas CHATWOOT_API_TOKEN en el .env (ver QUICK-START.md paso 3).
set -euo pipefail

cd "$(dirname "$0")/.."
set -a; [ -f .env ] && . ./.env; set +a

: "${CHATWOOT_API_TOKEN:?Falta CHATWOOT_API_TOKEN en .env (ver QUICK-START.md paso 3)}"
...
curl -sf -X POST "${BASE}/api/v1/accounts/${ACCOUNT_ID}/webhooks" \
  ...
  && echo -e "\nWebhook creado." \
  || echo -e "\nNo se pudo crear automaticamente. Crealo a mano (QUICK-START.md paso 3.4)."
